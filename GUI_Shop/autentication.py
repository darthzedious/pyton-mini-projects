from tkinter import Entry, Button  # windows from tkinter import Button
...
def render_entry():  # creating the front page login and register buttons
...
def register():  # creating the registration windows and button
    clean_screen()
...
def registration():  # after the register button is clicked the function is triggered
    info_dict = {
    ...
    if check_registration(info_dict):  # if registration is successful the user data is saved in our db json file.
...
def check_registration(info_dict):
    frame.delete("error")  # this line clears the previous "error" messages because they don't clear themselves.

    for key, value in info_dict.items():
        if not value.strip():  # if the user tries to submit without typing the required information.
            frame.create_text(
                200,
                300,
    ...
    for user in users_data:
        #print("User:", user)
        #print("No 'Username' key in user:", user)
...
login_button["state"] = "disabled"  # by default does not allow to button to be clicked
